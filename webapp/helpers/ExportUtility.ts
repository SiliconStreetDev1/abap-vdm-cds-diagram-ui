 ...
 * safely rasterize complex vector graphics into PNG blobs. Includes dynamic memory 
 * management for massive enterprise diagram exports and cross-origin taint scrubbing.
 */
...
export default class ExportUtility {
    ...
    /**
     * @public
     * @description Safely serializes an SVG to a PNG Blob. Implements a dynamic Canvas Governor 
     * to auto-downscale the resolution if the raw pixel array exceeds browser memory limits.
     * Scrubs external resources (CSS imports, external images) prior to rendering to prevent 
     * Canvas Tainting (SecurityError) which would permanently lock the export data.
    ...
    /**
     * @public
     * @description Prepares an SVG for raw standalone download. Inlines computed CSS styles 
     * from the browser DOM to preserve coloring in offline viewers, and recalculates the 
     * ViewBox to correctly capture negative coordinate spaces generated by engines like Graphviz.
    ...
    /**
     * @private
     * @description Safely converts a UTF-8 string to Base64 without using the deprecated `unescape` method.
