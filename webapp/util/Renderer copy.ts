...
/**
     ...
     * FIX: Overrides Mermaid's default safety limits (maxEdges, maxTextSize) to allow massive VDM generation.
            ...
            try {
                ...
                if (!this._bMermaidInit) {
                    mermaid.mermaidAPI.initialize({ 
                        ...
                        // ENTERPRISE SCALE FIX: Override default circuit breakers for massive diagrams
                        maxTextSize: 500000, // Bypass default 50k character limit
                        maxEdges: 10000,     // Bypass default 500 relationship line limit
                        
                        // Prevent Mermaid from squishing massive diagrams into a tiny 100% width box
                        flowchart: {
                            useMaxWidth: false,
                            htmlLabels: false // Enforce security fix specifically for flowcharts as well
                        }
                    });
                ...
                // UX POLISH: Clear any leftover error blocks Mermaid might have injected 
                // directly into the DOM on a previous failed run before we attempt to render again.
                if (oTarget) {
                    oTarget.innerHTML = "";
                }
                ...
                // Catch internal Mermaid parser errors and route them to the Fiori UI MessageStrip.
                // Mermaid often stores specific syntax errors in the 'str' property.
                fnOnError(`Mermaid Syntax/Render Error: ${e.str || e.message || e}`);
            }
        }).catch((oNetworkError: any) => {
            // Handle offline or CDN blocking scenarios gracefully
            fnOnError(`Failed to load Mermaid CDN: ${oNetworkError.message || oNetworkError}`);
        });
    }
  ...
  /**
     * Converts a raw SVG element into a PNG Blob.
     * FIX: Scrubs external CSS to prevent tainting AND dynamically scales to prevent memory limit crashes.
     */
    public static convertSvgToPng(oSvg: SVGSVGElement): Promise<Blob> {
        return new Promise((resolve, reject) => {
            let sSvgData = new XMLSerializer().serializeToString(oSvg);
            
            // Scrub external resources to prevent Canvas Taint (SecurityError)
            ...
            // Extract raw SVG dimensions
            const width = parseFloat(oSvg.getAttribute("width") || "3000");
            const height = parseFloat(oSvg.getAttribute("height") || "3000");
            
            // ====================================================================
            // ENTERPRISE HARDENING: Dynamic Canvas Governor
            // ====================================================================
            let scale = 2; // Start by requesting a high-res (Retina) export
            
            // Hard browser limits (Approx 16,000px dimension or 100 Megapixels total area)
            const MAX_DIMENSION = 16000; 
            const MAX_AREA = 100000000; 
            
            // Dynamically downscale if the resulting canvas would crash the browser
            while ((width * scale > MAX_DIMENSION || height * scale > MAX_DIMENSION || (width * scale * height * scale) > MAX_AREA) && scale > 0.5) {
                scale -= 0.5; // Step down the quality until it fits in memory
            }
                ...
                try {
                    canvas.toBlob((blob) => {
                        if (blob) {
                            ...
                            // If it still returns null, the diagram is just fundamentally too massive for PNG.
                            reject(new Error("Diagram is too massive for the browser to render as pixels. Please use 'Download SVG' instead."));
                        }
                    }, "image/png");
                } catch (e: any) {
            ...
            img.onerror = () => {
                reject(new Error("Failed to parse the sanitized SVG Data URI into the image object."));
            };
